...
void fft_print_timing_report(const grid_mpi_comm comm) {
  ...
  if (timers_initialized) {
    ...
    if (grid_mpi_comm_rank(comm) == 0) {
      ...
      grid_mpi_bcast_int(&number_of_timed_routines, 1, 0, comm);
      for (int routine = 0; routine < number_of_timed_routines; routine++) {
        // Exchange the length of the routine name
        int length = strlen(timed_routines[routine].routine_name) + 1;
        grid_mpi_bcast_int(&length, 1, 0, comm);
        // Exchange the actual routine name
        grid_mpi_bcast_char(timed_routines[routine].routine_name, length, 0,
                            comm);
        ...
        // ... and exchange them
        grid_mpi_sum_double_root(summed_info, 3, 0, comm);
        double max_info[2];
        max_info[0] = total_time;
        max_info[1] = self_time;
        grid_mpi_max_double_root(max_info, 2, 0, comm);
      ...
      grid_mpi_bcast_int(&number_of_routines_to_consider, 1, 0, comm);
      for (int routine = 0; routine < number_of_routines_to_consider;
           routine++) {
        int length = -1;
        grid_mpi_bcast_int(&length, 1, 0, comm);
        char *routine_name = malloc(length * sizeof(char));
        grid_mpi_bcast_char(routine_name, length, 0, comm);
        double total_time, self_time;
        int number_of_calls;
