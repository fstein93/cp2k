...
subs_C3=[[C3, 'calc_C3', rs]]
create_Routine_from_Function(C3_, file, max_deriv, subs_g0, [], constants_C3, 'calc_C3', [], rs)
...
class C4_(Function):
    ...
    @classmethod
    def eval(cls, rs):
        return -sympify('9/64')/rs**three*(g2z(rs)+D2(rs)-one/five/alpha**two/rs**two)
...
class C5_(Function):
    ...
    @classmethod
    def eval(cls, rs):
        return -sympify('9/40')/sqrt(two*pi)/rs**three*(g2z(rs)+D3(rs))
...
subs_Q=[[Q, 'calc_Q', rs]]
create_Routine_from_Function(Q_, file, max_deriv, [], [], constants_Q+constants_alpha, 'calc_Q', [mu,], x)
